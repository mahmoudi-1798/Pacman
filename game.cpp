...
void Game::update() {
    ...
    if(fall) {player.setDest(player.getDX(), player.getDY()+GRAVITY);}
...
void Game::loadMap(const char* fileName){
    Object tmp;
    tmp.setImage("res/pamap3.png", ren);
    ...
    for(int i=0; i<mh; i++){
        for(int j=0; j<mw; j++){
            ...
            if(current != 0){
                ...
                tmp.setId(current);
                if(current == 12 || current == 35 || current == 30 || current == 22) {
                    tmp.setSolid(0);
                }
                map.push_back(tmp);
            }
...
bool Game::colision(Object a, Object b){
    if((a.getDX() < (b.getDX()+b.getDW())) && ((a.getDX()+a.getDW()) > b.getDX())
    && (a.getDY()) < (b.getDY() + b.getDH()) && (a.getDY() + a.getDH()) > b.getDH()){
        return true;
    } else {
        return false;
    }
